...
export class AllocationsComponent implements OnInit, AfterViewInit {
  ...
  columns: any = [
    ...
    { field: 'item', name: 'Item', width: 75, baseWidth: 13 },
    { field: 'revision', name: 'Revision', width: 75, baseWidth: 7 },
    ...
    { field: 'batch', name: 'Batch', width: 75, baseWidth: 7 },
    { field: 'allocatedQty', name: 'Allocated Qty', width: 75, baseWidth: 10 },
    { field: 'UOM', name: 'UOM', width: 75, baseWidth: 6 },
    { field: 'workOrder', name: 'Work Order', width: 75, baseWidth: 9 },
    { field: 'woLine', name: 'WO Line', width: 75, baseWidth: 7 },
    { field: 'dropLg', name: 'Drop LG', width: 75, baseWidth: 8 },
    { field: 'dropLocator', name: 'Drop Locator', width: 75, baseWidth: 9 }
  ]
  constructor(
    ...
    private woService: WorkOrderService
      ...
      this.router.params.subscribe(params => {
        if (params.waveId && params.waveIuId) {
          ...
          this.parameterData = [];
          this.woService.getAllocations(body).subscribe((data:any) =>
          {
            if (data.status === 200) {
              if (!data.message) {
              this.waveNumberInput = data.result[0].waveNumber;
              this.IuInput = data.result[0].iuCode;
  ...
  backToEdit(){
    this.route.navigate(['wavemfg/editwave/'+ this.waveId]);
  }
